import { createIntlDict } from '@/i18n';

export const cfcDict = createIntlDict(
  {
    TITLE: 'Staff Sergeant / Language Specialist',
    CATEGORY: 'Military Service',
    DIVISION: 'ROKAF SSgt',
    AFFILIATION: 'R.O.K. - U.S. Combined Forces Command',
    LOCATION: 'USAG Yongsan-Casey, South Korea',
    DETAILS: [
      'Simultaneous English/Korean interpretation without script on a daily basis',
      'Meeting interpretation on site, over phone and over video conference calls in the presence of major(O-6) - major general(O-8) ranking officers from various units subordinate to ROKA, ROKN, ROKAF, ROKMC and USFK (United States Forces Korea)',
      'Translation of military documents including intelligence analysis reports',
    ],
  },
  {
    TITLE: '어학병',
    CATEGORY: '군 복무',
    DIVISION: '대한민국 공군 병797기 병장 만기제대',
    AFFILIATION: '한미연합군사령부',
    LOCATION: '서울 용산',
  }
);
